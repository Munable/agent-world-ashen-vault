...
ROOMS = {
 ...
 'gate': {'name':'旧门厅','position':[4,3],'neighbors':['camp','fork'],'text':'石碑记载旧墓库的通行誓约。认真阅读后才能辨认守卫的职责。'},
 ...
 'guard': {'name':'守卫前厅','position':[10,3],'neighbors':['fork','shrine'],'text':'守卫要求通行费。可以支付、交涉，或明确挑战。守卫以击昏入侵者为目标。'},
 'shrine': {'name':'余烬祭坛','position':[12,3],'neighbors':['guard'],'text':'失落的火种等待被带回。完成委托后，这里可以进行自愿的守印考验。'},
}
...
def new_campaign(role_id: str) -> dict:
    ...
    hero.update(source='Audited Fighter 1-2 build',srd_pages=[23,47,48,86,87,90,91,92,95,185,187,192,193],id='hero',name='守路旅人',team='hero',size='small',nimble=True,lucky=True,ac=19,max_hp=12,hp=12,
