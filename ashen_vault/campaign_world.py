...
def join(ctx,args):
    ...
    state=new_campaign(ctx.actor_role_id);ctx.set_state('campaign:'+ctx.actor_role_id,'state',state,expected_version=0)
...
def bootstrap(ctx):return {'scene':view(load(ctx)),'rules_scope':'G1 constrained solo Fighter 1-2 preview; not all SRD actions or full 3-class campaign.'}
...
SPECS=[FunctionSpec('adventure.join',join,schema(),description='Initialize your own campaign once. The identity already belongs to you; does not create a role or token.'),
...
register('level_up',{'style':{'enum':['defense','dueling']}},('style',),'At 300 XP select one audited level-2 build; no free heal or repeated rewards.')
...
WORLD=WorldDefinition('ashen-vault-ember','Ashen Vault: Lost Ember G1 preview',tuple(SPECS),state_rules=(StateRule('campaign:','state',STATE),),
    state_authorizer=owned,bootstrap=bootstrap,views=(ViewSpec('adventure',scene,timeline=True),),
    entry_instructions='Use the user-held token for this universe. Read adventure.look, join once, then use current offered tools/revision. '
    'Fighter 1-2 constrained solo campaign with authored NPC policies; not complete SRD. Server rolls and resolves all actions. '
