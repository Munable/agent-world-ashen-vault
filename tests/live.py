...
class LiveServer:
    def __enter__(self):
        ...
        runtime=WorldRuntime(self.db); install_world(runtime,'ashen-vault',WORLD)
        self.tokens={}
        for seat in ('warden','sentinel'):
            role=runtime.create_role(seat)['role_id']
            self.tokens[seat]=runtime.issue_identity_token('ashen-vault',role)['token']
    ...
    def start(self):
        self.proc=subprocess.Popen([sys.executable,'-u','-m','ashen_vault.server','--db',str(self.db),'--port',str(self.port)],
